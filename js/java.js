setTimeout(function () {
    Java.perform(function () {
        ...
        //将Socket类的connect方法进行Hook并重写
        socket.connect.overload('java.net.SocketAddress', 'int').implementation = function (addr, timeout) {
            // var ret = this.connect(addr, timeout);
            if (addr.toString().endsWith(":443")) { //判断连接地址是否以":443"结尾，即是否连接到HTTPS服务
                console.log(addr.toString())
                //创建一个新的InetSocketAddress对象(inetSockAddrImpl)，并指定代理服务器地址和端口
                var inetSockAddrImpl = inetSockAddrWrap.$new(proxy_addr4, 8081) 
                var ret = this.connect(inetSockAddrImpl, timeout); //调用原始的connect方法进行连接，并将返回值保存在变量ret中
                console.log(addr.toString());
                }
            ...
            } else {//直接调用原始的connect方法进行连接，并将返回值保存在变量ret中。 
                var ret = this.connect(addr, timeout);
            }
